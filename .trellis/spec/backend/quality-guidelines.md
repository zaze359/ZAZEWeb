...
Spring Boot's dependency-management BOM (`io.spring.dependency-management`) is authoritative for managed libraries. The managed values below are from **Spring Boot 2.7.18** (`okhttp` 4.9.3, `h2` 2.1.214, `gson` 2.9.1, `logback` 1.2.12, `snakeyaml` 1.30, `spring-security` 5.7.11). Three traps observed in practice:

- **Transitive downgrade.** `core:network` declares `api("com.squareup.okhttp3:okhttp:4.10.0")`, and its own configuration resolves 4.10.0. But when another module inherits okhttp *transitively* through `core:network`, the BOM constraint downgrades it to **4.9.3**. Fix: any module that uses OkHttp directly must declare it explicitly — `implementation("com.squareup.okhttp3:okhttp:4.10.0")` — exactly as the root app, `core:network` and `feature:appmarket` already do. This keeps compile-time aligned with the runtime jar (which ships 4.10.0).
- **The root project's BOM overrides submodule declarations.** A version pinned in a submodule (e.g. `core:database` → `h2:2.3.232`) is only a *transitive* dependency from the root app's point of view, so `io.spring.dependency-management` re-applies the BOM value to it. `./gradlew dependencies` still prints the new version, but **the version actually shipped inside the boot jar is the BOM one**. Fix: pin it in the **root** `build.gradle.kts` as well (see the `安全版本覆盖` block there).
  > **Verify with `unzip -l build/libs/*.jar | grep BOOT-INF/lib/<artifact>` — never trust the `dependencies` output alone.** This bit us on 2026-09-25: gson/h2/spring-security-crypto were pinned in submodules only, and the jar still shipped 2.9.1 / 2.1.214 / 5.7.11.
  > Cleaner alternative (not yet adopted): override the BOM property in the root script — `extra["h2.version"] = "2.3.232"` — or use a `dependencyManagement { dependencies { dependency(...) } }` block. Every artifact above has a matching property. This avoids the root+submodule duplication, which drifts silently when only one side is edited.
- **H2 version.** `core:database` **and the root** now both pin `com.h2database:h2:2.3.232` (runtime), so the shipped jar really is 2.x and 2.x-only URL options are fine. H2 is on the runtime classpath already — no need to add it for local smoke runs. (Before 2026-09-25 only the submodule pinned it, and the BOM silently resolved the runtime H2 to 1.4.200.)
...
- **应用宝 does support server-side name search (corrected 2026-09-23 — an earlier version of this note said the opposite and was wrong).** The endpoint fired while typing on the web search page is `POST https://yybadaccess.3g.qq.com/v2/dc_pcyyb_official`: Content-Type **must** be `text/plain;charset=UTF-8`; `head.hostAppInfo.scene` **must** be `search_result` (any other value returns empty); `body.listS.keyword.repStr = ["<keyword>"]`; `body.layout = yybn_search_result_list`. Results are in `data.components[].data.itemData[]`. It returns a **fixed Top-3** regardless of `size`, Chinese keywords match exactly on the first hit, and the payload is only ~8KB. 小米/华为(signed request, `rtnCode 1002`)/酷安(403)/OPPO/vivo still have no usable server-side search — do **not** scrape their search pages.
...
- **OkHttp is capped at 4.10.0 by the Kotlin compiler.** okhttp 4.11+ (including 4.12.0) is compiled with Kotlin 1.8/1.9; against this project's Kotlin 1.6.21 compiler it fails with `Class 'okhttp3.Request' was compiled with an incompatible version of Kotlin. The binary version of its metadata is 1.8.0, expected version is 1.6.0`. The same applies to okio ≥ 3.4.0. Do not bump either one without first upgrading the Kotlin Gradle plugin.
